...
while ~stop_criterium
    ...
    for pattern = 1:size(input_data,1)
        ...
        err = goal(pattern, :)' - output;
        output_error = 0.5 * err;
        
        % Compute local gradient of output layer
        local_gradient_output = d_sigmoid(output_activation) .* (err);

        % Compute the error on the hidden layer (backpropagate)
        hidden_error = 0.5 * (goal(pattern, :)' - hidden_output);        
        
        % Compute local gradient of hidden layer
        local_gradient_hidden = d_output_function(hidden_activation) .* (w_output * local_gradient_output')';
        ...
        delta_hidden = learn_rate .* input_data(pattern, :)' * local_gradient_hidden;
